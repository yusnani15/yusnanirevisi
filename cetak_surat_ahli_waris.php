...
<?php
include("koneksi.php");
$query_surat =mysqli_query($db,"select * from tb_surat_ahli_waris where nomor='".$_GET["nomor"]."'");
$nomor="-";	
$tanggal="-";
$periode="-";
$nik="-";
...
if (mysqli_num_rows($query_surat)>0){
	...
	$query_pemilik =mysqli_query($db,"select * from tb_penduduk where nik='".$data_surat["pemilik"]."'");
	$pemilik="-";
	if(mysqli_num_rows($query_pemilik)>0){
		$data_pemilik=mysqli_fetch_array($query_pemilik);
		$pemilik=$data_pemilik["nama"];
		$pekerjaan=$data_pemilik["pekerjaan"];
	}
			...
			<?php include("kop_surat.php");?>
				<table width="590" border="0" align="center" cellpadding="3" cellspacing="3">
				  <tr>
					...
					<center>
						<u><b>SURAT KETERANGAN AHLI WARIS</b></u><br>
						Nomor : <?php echo "470/".$nomor."/".$periode;?><br>
						<br>
					</center>
						Pangulu Nagori Parasmian Kecamatan Dolok Silau Kabupaten Simalungun, dengan ini menerangkan sesuai dengan Surat Pernyataan Ahli waris tanggal 
						<?php echo $tanggal; ?> (terlampir) dibawah ini adalah benar Ahli Waris dari <?php echo $pemilik; ?> yang bertempat tinggal di 
						Nagori Perasmian masing-masin yaitu :<br>
						<?php
						$query_wars=mysqli_query($db,"select * from tb_ahli_waris_detail where nomor='".$_GET["nomor"]."'");
						$no=0;
						while($data_wars=mysqli_fetch_array($query_wars)){
							$query_penduduk =mysqli_query($db,"select * from tb_penduduk where nik='".$data_wars["nik"]."'");
							$nama="-";
							$tempat_lahir="-";
							$tanggal_lahir="-";
							$nik="-";
							...
							<table width="98%" border="0" align="center" cellpadding="0" cellspacing="2">
								<tr>
									<td width="20" rowspan="7" valign="top"><?php $no=$no+1; echo $no; ?>.</td>
						...
						<?php } ?>
						<br>
						Bahwa nama-nama tersebut diatas adalah benar ahli waris yang sah dari <?php echo $pemilik; ?> dan tidak ada lagi ahli waris lainnya
						selain nama-nama tersebut diatas.<br><br>

						Demikian Surat keterangan Ahli Waris ini diperbuat dengan sebenarnya untuk dapat dipergunakan seperlunya.
						<br><br>
